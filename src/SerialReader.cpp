 ...
 *
 *        prompt   - the interact message and buf should be allocated already and return true.
 *        buf      - a part of memory which is already allocated to save string read from serial
 *        maxLen   - the buf len, which should be upper bound of the read-in string's length, this should > 0
...
bool readFromSerial(char *prompt, char *buf, int maxLen, int timeout)
{
	...
	while (true) {
		...
		if (len > maxLen) {
			Serial.printf(
				"Your input should less than %d character(s), now you input %d characters.\r\n",
				maxLen,
				len);
		}
		else if (len > 0) {
